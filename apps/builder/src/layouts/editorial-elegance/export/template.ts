 ...
 */
function generateBodyHTML(invitation: InvitationData, translations?: Record<string, unknown>): string {
  ...
  const wedding = data?.wedding || {};
  const venue = wedding?.venue || {};
  const editorialIntro = (data as Record<string, unknown>)?.editorialIntro as Record<string, unknown> | undefined || {};
  const events = (data as Record<string, unknown>)?.events as { events?: Array<Record<string, unknown>> } | undefined;
  const eventsList = events?.events || [];
  const gallery = (data as Record<string, unknown>)?.gallery as { images?: Array<{ src: string; alt?: string }> } | undefined;
    ...
    html += `
      <section class="ee-section ee-events-section">
        ...
        <div class="ee-event-cards">
          ${eventsList.map((event: Record<string, unknown>) => `
