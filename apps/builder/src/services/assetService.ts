 ...
 */
async function retryWithBackoff<T>(fn: () => Promise<T>, retries: number = MAX_RETRIES, delay: number = INITIAL_RETRY_DELAY): Promise<T> {
  try {
    return await fn();
  } catch (error) {
    if (retries === 0) {
 ...
 */
export async function uploadImage(file: File, onProgress?: (progress: number) => void): Promise<{ url: string; asset: Asset }> {
 ...
 */
export async function uploadImages(files: File[], onFileProgress?: (fileIndex: number, progress: number) => void): Promise<UploadResult[]> {
...
export async function deleteImage(imageUrl: string): Promise<void> {
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({ error: 'Delete failed' })) as ErrorResponse;
...
export async function getAssets(): Promise<Asset[]> {
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({ error: 'Failed to fetch assets' })) as ErrorResponse;
...
export async function getAssetCountByUrls(urls: string[]): Promise<number> {
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({ error: 'Failed to get asset count' })) as ErrorResponse;
