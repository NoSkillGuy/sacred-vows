...
function ConfettiLayer() {
  useEffect(() => {
    ...
    function clearAllConfetti() {
      ...
      if (layer) {
        // Remove all existing confetti pieces
    ...
    // Periodic cleanup to catch any stuck pieces (fallback safety)
    const cleanupInterval = setInterval(() => {
      const layer = document.getElementById("confettiLayer");
      if (layer && layer.children.length > 0) {
        // Remove any pieces that might have gotten stuck
        // This is a safety net in case animationend events don't fire
        const now = Date.now();
