...
const ATTEMPT_THRESHOLD = 10; // Increased threshold to reduce false positives
const TIME_WINDOW_MS = 10000; // 10 seconds - longer window to reduce false positives
 ...
 * Note: This can have false positives, so we use a higher threshold and require both dimensions
 */
function detectByWindowSize(): boolean {
  ...
  // Increased threshold and require BOTH width AND height to be over threshold
  // This reduces false positives from browser chrome, scrollbars, etc.
 ...
 * Note: This method can have false positives, so we rely more on window size detection
 */
...
export function startDevToolsDetection(isProduction = true): void {
  ...
  // Use longer interval to reduce false positives
  ...
  }, 1000); // Increased from 500ms to 1000ms to reduce false positives
}
...
(function() {
  ...
  function detectDevTools() {
    // Only use window size detection to reduce false positives
